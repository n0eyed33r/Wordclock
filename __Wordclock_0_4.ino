...
CRGB leds[NUM_LEDS];
//Taster
const int TasterStunde = 5; // the number of the hour-pushbutton pin
const int TasterMinute = 3; // the number of the minute-pushbutton pin
// variables will change:
int TasterStundeStatus = 0; // variable for reading the hour-pushbutto status
int TasterMinuteStatus = 0; // variable for reading the minute-pushbutton status
//Indizes
int i = 0, n = 0 ;                     //indices

//Zeitintervall und Mitzähler
unsigned long intervalMin = 6000;  // the time we need to wait for a minute
unsigned long intervalHour = 360000;// the time we need to wait for an hour
...
void setup()
{
	...
	pinMode(TasterStunde, INPUT);
	pinMode(TasterMinute, INPUT);
}
...
unsigned long currentHourMillis = millis();
	...
	{		
		...
		{
		  i = 0; Serial.println("Stunde"); Serial.println(i+1);
		  word2stripe(wordhZwoelf, sizeof(wordhZwoelf) / sizeof(int), CRGB::Black);
		  word2stripe(wordhEin, sizeof(wordhEin) / sizeof(int), CRGB( 8, 75, 10));
		  FastLED.show(); Serial.println("Stunde"); Serial.println(i+1);
		  i++; previousHourMillis = currentHourMillis;
